...
export default function LoginPage() {
  const { firebaseUser, user, needsSetup, loading, signIn } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (loading) return;
    if (firebaseUser && needsSetup) {
      router.push("/setup");
    } else if (user) {
      router.push("/");
    }
  }, [loading, firebaseUser, user, needsSetup, router]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-sm text-center">
        ...
        <p className="text-sm text-gray-500 mb-8">
          Foldspace evaluation dashboard
        </p>
        <button
          onClick={signIn}
